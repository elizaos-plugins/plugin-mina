...
// src/actions/transfer.ts
import { Mina as Mina2, AccountUpdate, PublicKey as PublicKey2, UInt64 as UInt642 } from "o1js";
...
var WalletProvider = class {
  constructor(network, account, cacheManager) {
...
var transfer_default = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    try {
      ...
      const recipient = PublicKey2.fromBase58(transferContent.recipient);
      const adjustedAmount = UInt642.from(transferContent.amount).mul(
        MINA_UNIT
      );
...
import { Mina as Mina3, PublicKey as PublicKey3 } from "o1js";
function isFaucetContent(content) {
...
var faucet_default = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger2.log("Starting FAUCET handler...");
    const walletInfo = await walletProvider.get(runtime, message, state);
    state.walletInfo = walletInfo;
    let currentState;
    if (!state) {
    ...
    const FaucetContext = composeContext2({
      state: currentState,
    ...
    const FaucetContent = content.object;
    ...
    try {
      const recipient = PublicKey3.fromBase58(FaucetContent.recipient);
      await Mina3.faucet(recipient, FaucetContent.network);
...
import { PublicKey as PublicKey4, TokenId, fetchAccount as fetchAccount2 } from "o1js";
...
var balance_default = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger3.log("Starting Balance handler...");
    const walletInfo = await walletProvider.get(runtime, message, state);
    state.walletInfo = walletInfo;
    let currentState;
    if (!state) {
    ...
    const BalanceContext = composeContext3({
      state: currentState,
    ...
    const BalanceContent = content.object;
    ...
    try {
      const address = PublicKey4.fromBase58(BalanceContent.address);
      let account;
      if (!BalanceContent.token) {
        account = await fetchAccount2({ publicKey: address });
      } else {
        const tokenId = TokenId.fromBase58(BalanceContent.token);
        account = await fetchAccount2({
          publicKey: address,
          tokenId
        });
      }
      const balance = new BigNumber2(
...
var minaPlugin = {
  ...
  actions: [transfer_default, faucet_default, balance_default],
  evaluators: [],
  providers: [walletProvider]
};
var index_default = minaPlugin;
export {
  balance_default as BalanceMinaToken,
  faucet_default as FaucetMinaToken,
  transfer_default as TransferMinaToken,
  WalletProvider,
